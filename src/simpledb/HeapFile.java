...
public class HeapFile implements DbFile {
    ...
    private HashMap<Integer, Boolean> _freePage;
    ...
    public HeapFile(File f, TupleDesc td) {
        ...
        try {
        	...
        	_freePage = new HashMap<Integer, Boolean>();
        	markNonFreePages();
    ...
    private synchronized void markFree(int pageNumber, boolean isFree) {
    	_freePage.put(pageNumber, isFree);
    }
    
    private synchronized boolean isFree(int pageNumber) {
    	return _freePage.get(pageNumber);
    }
    ...
    */
    ...
    public Page readPage(PageId pid) {
        ...
        try {
        	...
        	assert (offset + BufferPool.PAGE_SIZE <= _fileChannel.size());
            _fileChannel.read(buffer, offset);
    ...
    public void writePage(Page page) {
    	...
    	try {
    		...
    		_fileChannel.write(buffer, offset);
    	} catch (IOException e) {
    ...
    public int numPages() {
    	try {
    		int pageCount = (int) Math.ceil(this._fileChannel.size() / BufferPool.PAGE_SIZE);
    		return pageCount;
    	} catch (IOException e) {
    ...
    private synchronized HeapPage getNextFreePage(TransactionId tid)
    	...
    	assert (numPages() == _freePage.size());
    	
    	for (int i = 0; i < this.numPages(); i++) {
    		if (isFree(i)) {
    			HeapPageId pid = new HeapPageId(this.getId(), i);
    			//System.out.println("Getting free page: " + i);
    ...
    public HeapPage deleteTuple(TransactionId tid, Tuple t)
    	...
    	HeapPage page = (HeapPage) pool.getPage(tid,  rid.getPageId(), Permissions.READ_WRITE);
