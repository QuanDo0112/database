 ...
 */
public class CustomLogTest extends SimpleDbTestBase {
    File file1;
    File file2;
    HeapFile hf1;
    HeapFile hf2;
    ...
    void insertRow(HeapFile hf, Transaction t, int v1, int v2)
        ...
        TestUtil.assertEquals(SystemTestUtil.SINGLE_INT_DESCRIPTOR, result.getTupleDesc());
        TestUtil.assertEquals(1, ((IntField)result.getField(0)).getValue());
        TestUtil.assertFalse(insert.hasNext());
        insert.close();
    }
    ...
    void setup()
     ...
     public void PatchTest()
        ...
        if(Database.getLogFile().getTotalRecords() != 4) {
            System.out.println("Got number of logs instead of 4: " + Database.getLogFile().getTotalRecords());
