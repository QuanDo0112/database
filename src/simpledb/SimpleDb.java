...
public class SimpleDb {
    ...
    private boolean convertArgs(String arg) {
    	return arg.equalsIgnoreCase("convert");
    }
    
    private boolean print(String arg) {
    	return arg.equalsIgnoreCase("print");
    }
    
    private boolean invokeParser(String arg) {
    	return arg.equalsIgnoreCase("parser");
    }
    
    public void parseArgs(String[] args) throws Exception, TransactionAbortedException {
    	int argLength = args.length;
    	assert (argLength > 0);
    	String command = args[0];
    	
        if (convertArgs(command)) {
        	convertFile(args);
        } else if (print(command)) {
            printTable(args);
        } else if (invokeParser(command)) {
            parseStatement(args);
        } else {
        	usage(command);
        }
	...
	private void usage(String command) {
		...
		System.err.println("Valid commands are: convert, print, parser");
        System.exit(1);
	}

	private void parseStatement(String[] args) {
	...
	private void printTable(String[] args) throws DbException,
	...
	private void convertFile(String[] args) throws IOException {
		if (args.length == 3) {
    ...
    public static void main (String args[])
            throws DbException, TransactionAbortedException, IOException, Exception {
